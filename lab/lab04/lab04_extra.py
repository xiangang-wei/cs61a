...
def insert_sort(lst,num):
    ...
    while i < len(lst):
        if num >= lst[i]:
            ...
            break
        i+=1
    if len(result) == len(lst): # when num is the largest element in lst
        result.append(num)
    return result
...
def replace_elem(lst, index, elem):
    ...
    # it seems impossible to make the function with one return statement
    if index == len(lst)-1:
        return lst[0:index]+[elem]
    else:
