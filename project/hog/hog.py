...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    # 1. calaulate the number of rolling dice of current player
    # 2. calculate current turn points of the player, considering whether rolling number equals 0 or not
    # 3. swap check
    # 4. check whether game ends or not
    while True:
        if player == 0:
        ...
        score = score + take_turn(strategy(score,opponent_score),opponent_score,dice)
    ...
    def say(score0,score1):
        ...
        if current_high > previous_high:
            if current_score > 1:
                ...
                print(current_score,"point! That's the biggest gain yet for Player",who)
        else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    result = 0
    while num <= 10:
        averae_dice = make_averaged(roll_dice,num_samples)
        score = averae_dice(num,dice)
        if score > max:
            max = score
            result = num
        num += 1
